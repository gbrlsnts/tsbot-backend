...
import { categoryAlreadyExists, categoryHasConfigs } from 'src/shared/messages/server.messages';
...
export class GroupCategoryService {
  ...
  getAllCategoriesByServer(serverId: number, withGroups = false): Promise<GroupCategory[]> {
    ...
    if(withGroups) {
      builder
        ...
        .leftJoinAndMapOne('group', ServerGroup, 'g', 'conf.groupId = g.id');
    }
  ...
  async createCategory(serverId: number, dto: CategoryDto): Promise<GroupCategory> {
    try {
  ...
  async updateCategory(id: number, dto: CategoryDto): Promise<GroupCategory> {
    try {
